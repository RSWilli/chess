...
{{ range $name, $data := .Parts -}}
var {{ $name }}MoveMasks = squareLookup[BitBoard]{
	store: [64]BitBoard{
		{{ range $chunk := chunk $data.MoveMasks 8 }}
		{{- range $m := $chunk }}
		{{- printf "%#018x," $m -}}
		{{- end }}
		{{ end -}}
...
var {{ $name }}MoveTargets = squareLookup[hashTable]{
	store: [64]hashTable{
		{{ range $m := $data.Magics -}}
		{
			magic: {{ printf "%#018x" $m.Magic }},
			bits: {{ printf "%#018x" $m.Bits }},
			data:  []BitBoard{
				{{ range $chunk := chunk $m.Values 10 }}
				{{- range $v := $chunk }}
				{{- printf "%d: %#018x," $v.Key $v.Value -}}
				{{- end }}
				{{ end -}}
			},
		},
...
func main() {
	...
	tpl := template.Must(template.New("tpl").Funcs(funcs).Parse(srcTemplate))
	...
	for i := range 64 {
		...
		slices.SortFunc(rookMagicData.Values, func(a, b entry) int {
			return cmp.Compare(a.Key, b.Key)
		})
		slices.SortFunc(bishopMagicData.Values, func(a, b entry) int {
			return cmp.Compare(a.Key, b.Key)
		})
...
func chunk(data any, size int) iter.Seq[any] {
	switch arr := data.(type) {
	case []entry:
		iter := slices.Chunk(arr, size)
